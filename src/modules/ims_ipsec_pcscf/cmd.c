		...
		pcontact_info_t *ci, sip_msg_t *m, tm_cell_t *t, int sflags)
{
	...
	if(m->first_line.type == SIP_REQUEST) {
		...
		req_msg.buf = (char*) pkg_malloc((t->uas.request->len + 1) * sizeof(char));
		...
		str remote_addr, ipsec_t *s, unsigned short received_port)
{
	...
	remove_sa    (sock, remote_addr, ipsec_addr, s->port_uc, s->port_pc, s->spi_ps,
			ip_addr.af);
	remove_policy(sock, remote_addr, ipsec_addr, s->port_uc, s->port_pc, s->spi_ps,
			ip_addr.af, IPSEC_POLICY_DIRECTION_IN);

	// SA6 P-CSCF client to UE client
	remove_sa    (sock, ipsec_addr, remote_addr, s->port_pc, s->port_uc, s->spi_us,
			ip_addr.af);
	remove_policy(sock, ipsec_addr, remote_addr, s->port_pc, s->port_uc, s->spi_us,
			ip_addr.af, IPSEC_POLICY_DIRECTION_OUT);

	// SA7 P-CSCF server to UE server
	remove_sa    (sock, ipsec_addr, remote_addr, s->port_ps, s->port_us, s->spi_uc,
			ip_addr.af);
	remove_policy(sock, ipsec_addr, remote_addr, s->port_ps, s->port_us, s->spi_uc,
			ip_addr.af, IPSEC_POLICY_DIRECTION_OUT);

	// SA8 UE server to P-CSCF server
	remove_sa    (sock, remote_addr, ipsec_addr, s->port_us, s->port_ps, s->spi_pc,
			ip_addr.af);
	remove_policy(sock, remote_addr, ipsec_addr, s->port_us, s->port_ps, s->spi_pc,
			ip_addr.af, IPSEC_POLICY_DIRECTION_IN);

	// Release SPIs
	// TODO: Only release the client and the server ports if flag release_proxy_ports is set
	...
	if((ci.via_port == SIP_PORT) ||
	   (pcontact->security_temp->data.ipsec->port_ps == 0  &&
		...
		if(req_sec_params == NULL) {
			...
			if (req_sec_params->data.ipsec->port_uc != pcontact->security_temp->data.ipsec->port_uc ||
			    req_sec_params->data.ipsec->port_us != pcontact->security_temp->data.ipsec->port_us ||
			    req_sec_params->data.ipsec->spi_uc != pcontact->security_temp->data.ipsec->spi_uc ||
			    req_sec_params->data.ipsec->spi_us != pcontact->security_temp->data.ipsec->spi_us) {
